...
def load_Data(paths: str) -> pd.DataFrame():
    ...
    filtered_data = data[(data['distance_from_origin'] >= min_distance) & (data['distance_from_origin'] <= max_distance)].copy()
...
df['Z_upd'] = df['z'].apply(denormalize, t_min=target_min, t_max=target_max, n_min=custom_round(min(df['z'])), n_max=custom_round(max(df['z'])))
df['Y_upd'] = df['y'].apply(denormalize, t_min=target_min, t_max=target_max, n_min=custom_round(min(df['y'])), n_max=custom_round(max(df['y'])))
df['X_upd'] = df['x'].apply(denormalize, t_min=target_min, t_max=target_max, n_min=custom_round(min(df['x'])), n_max=custom_round(max(df['x'])))
...
button = tk.Button(root, text="Close", command=lambda: (cv2.destroyAllWindows(), root.destroy())) # Call cv2.destroyAllWindows and root.destroy functions when button is clicked
